...
public class ToRandomTeamSelection : MonoBehaviour
{
    private int selectedTeamIndex;
    private Color desiredColor;
    private int oppRandomIndex;
    ...
    public void Confirm()
    {
        PlayerPrefs.SetString("TeamName", teamList[selectedTeamIndex].teamName);
        PlayerPrefs.SetInt("TeamSelected", selectedTeamIndex);
        
        oppRandomIndex = Random.Range(0,23);
        while (oppRandomIndex == selectedTeamIndex)
        {
            oppRandomIndex = Random.Range(0,23);
        }
        PlayerPrefs.SetInt("OppRandomTeamSelected", oppRandomIndex);

        AudioManager.Instance.StopMusic(characterSelect);
        SceneManager.LoadScene("QuickMatchShowcase");
